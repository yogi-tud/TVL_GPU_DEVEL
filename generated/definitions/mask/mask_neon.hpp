...
namespace tvl {
   ...
   namespace details {
      template<  ImplementationDegreeOfFreedom Idof >
      struct insert_delim_zero_impl< simd< int64_t, neon >, Idof > {
         using Vec = simd< int64_t, neon  >;
         static constexpr bool native_supported() {
            return false;
         }
   /*
    * @brief Inserts 0 into bitmask using provided mask.
    * @details todo.
    * @param data Data mask register (e.g., data=0b11001011).
    * @param mask Vector mask register containing mask which is used to insert 0 bits (e.g., mask=0b110011)
    * @return Data mask with inserted 0 bits (e.g., 0010{from data}00{because mask is 0 }11{from data}).
    */
         [[nodiscard]] TVL_NO_NATIVE_SUPPORT_WARNING
         TVL_FORCE_INLINE static typename Vec::mask_type apply(
            typename Vec::mask_type  data, 
            typename Vec::mask_type  mask
         ) {
            static_assert( !std::is_same_v< Idof, native >, "The primitive insert_delim_zero is not supported by your hardware natively while it is forced by using native" );
            
            return ( ( vec_mask[ 1 ] >> 62 ) & 0b10 ) | ( vec_mask[ 0 ] >> 63 );
         }
      };
   } // end of namespace details for template specialization of insert_delim_zero_impl for neon using int64_t.
